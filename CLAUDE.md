...
| Bandeau de pièce + barre de contrôle basse (communs Répétition/Enregistrement) | composants `src/shared/PlayHeader.jsx` (marque + titre de la pièce, repliable) et `src/shared/ProgressBar.jsx` (slider indexé) ; CSS dans `theme.css` (`.play-header*`, `.controls`, `.ctrl-btn`, y compris `.ctrl-btn.my-jump` pour les sauts « ma réplique », communs aux deux pages…). Pas de tiret cadratin dans les headers. `PlayHeader` sert aussi à l'Édition et à l'Avancement : les quatre pages qui connaissent la pièce affichent son titre en haut, au même endroit et dans la même serif. **Où vit le lien vers l'accueil** : plus dans la rangée du haut. Le bouton de repli avale toute cette rangée, marque « PrettyDrama » comprise (en simple texte, plus un `<a>`), et le lien vit **en pied du bandeau déplié, centré** : le logo aux deux masques sur sa pastille, SEUL, sans un mot (`.play-header-home`, `homeHref` donc `respo.html` depuis l'Édition et l'Avancement). Il porte donc lui-même le nom accessible (`aria-label`/`title` « Accueil PrettyDrama »), le sceau restant décoratif. Deux filets courts qui s'effacent vers l'extérieur l'encadrent (`.play-header-foot`) : seul et centré, il avait l'air d'une pastille tombée là ; encadré, il ferme le bandeau comme un cul-de-lampe ferme une page (dégradés et courts plutôt qu'une règle pleine d'un bord à l'autre, qui aurait pesé sous des réglages déjà chargés). Son survol est **celui de toutes les surfaces cliquables du site** (cartes de l'accueil, bouton de dépôt), ses trois effets compris : filet à la couleur de marque, `--shadow-hover`, un pixel vers le haut. Pas une trouvaille par bouton. Le filet est posé transparent dès le repos (sinon la pastille se réduirait d'un pixel au passage de la souris) et prend `--header-accent`, pas `--accent` : même vin, mais le second est re-skinné en violet par l'éditeur. `--shadow-hover` et pas `--shadow` pour la même raison, l'éditeur met le second à `none`. Deux pistes écartées : inverser la pastille (fond vin, dessin sable) noie les masques, qui peignent leurs deux visages en `--page-mark-soft`, et il ne reste qu'une tache pâle ; l'agrandir la redessine à une taille qu'elle ne supporte pas. Bague de focus de la même géométrie que `--focus-ring` mais bâtie sur `--header-accent`, la bague partagée dérivant de `--accent`, que l'éditeur re-skinne. Sur mobile le pouce vise le haut de la barre pour la replier et tombait sur la marque : le geste le plus courant de la page renvoyait au menu principal, en perdant le personnage choisi et, sur l'Enregistrement, les prises non exportées (le `LeaveGuard` prévenait, mais l'accident restait quotidien). Maintenant tout ce que le doigt atteint dans la rangée replie, et sortir de la page demande de déplier d'abord. En pied et pas en tête du bandeau : une sortie de page ne se met pas au-dessus des réglages de la page. Le sceau, lui, reste hors du bouton : son `role="img"`/`aria-label` brouillerait le nom accessible du bouton, et c'est la seule zone inerte de la rangée (elle l'a toujours été). **Corollaire : le bandeau se replie sur les quatre pages, y compris l'Avancement**, qui n'a pourtant aucun réglage ; une page qui ne se replierait pas serait la seule à garder son bandeau ouvert sous le pouce. Son bandeau à elle ne contient qu'une phrase (`.header-hint` dans `src/dashboard/App.jsx` : à quoi sert la page, jamais comment lire la grille, ça c'est le rôle de `.dash-legend` sous le tableau) et le logo de retour : déplier pour ne trouver qu'un logo faisait vide. D'où aussi l'infobulle unique du bouton de repli, « Déplier ou replier le bandeau » et non « les réglages » : une formule par contenu se désaccorderait au premier ajout. Seul `PageHeader` (titre = libellé de page) reste pour les écrans qui n'ont pas encore le manifest, via `PageState` |
