...
import { OutlineIcon, PersonIcon, SearchIcon } from "../shared/icons.tsx";
...
// The three sections, and the only values `section` ever takes.
export type RailSection = "structure" | "characters" | "search";

const SECTIONS: { key: RailSection; Icon: () => React.JSX.Element }[] = [
...
const clampPanel = (px: number) => Math.max(MIN_PANEL, Math.min(MAX_PANEL, Math.round(px)));
...
export default function EditorRail({
  ...
  onSection: (section: RailSection | null) => void;
  structure: ReactNode;
  characters: ReactNode;
  search: ReactNode;
}) {
  const tabRefs = useRef<Partial<Record<RailSection, HTMLButtonElement | null>>>({});
  ...
  const close = useCallback(() => {
    ...
    const tab = section === null ? null : tabRefs.current[section];
    onSection(null);
    tab?.focus();
  }, [section, onSection]);
  ...
  return (
    <aside
      ...
      /* A CSS custom property is not in `CSSProperties`, hence the cast. */
      style={{ "--ed-rail-panel": `${panelWidth}px` } as CSSProperties}
    >
      ...
      {current && (
        ...
        <div
          ...
          onKeyDown={(e) => {
            ...
            // MANDATORY, same guard as useSearch.ts: a `ConfirmModal` opened from the
