...
import { buildReplaceEdits, replaceOneEdit, searchScript } from "./search.ts";
import type { Match } from "./search.ts";
import type { ScriptAction } from "./reducer.ts";
import type { Script } from "../shared/types.ts";

// Where the reader is, as a POSITION in the play and not a rank: it survives a change
// of query, which a rank does not.
interface Anchor {
  lineId: string;
  lineOrdinal: number;
  start: number;
...
export default function useSearch({
  ...
  goToMatch: (match: Match, focus?: boolean) => void;
  isOpen: boolean;
  onOpen: () => void;
  onClose: () => void;
  enabled: boolean;
  ...
  const [current, setCurrent] = useState<Anchor | null>(null);
  ...
  const anchorOn = (match: Match, start = match.start): Anchor => ({
    lineId: match.lineId,
  ...
  // SEPARATE from the undo/redo effect (App.tsx), which resubscribes on every edit:
...
function seedFromSelection(setQuery: (seed: string) => void) {
  const el = document.activeElement;
  // The tag test is what makes the cast safe: only a textarea carries a selection.
