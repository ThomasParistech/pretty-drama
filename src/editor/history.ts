...
// `present !== saved` (App.tsx), an IDENTITY comparison.
import { EMPTY_SCRIPT, scriptReducer } from "./reducer.ts";
import type { ScriptAction } from "./reducer.ts";
import type { Script } from "../shared/types.ts";

// The stack. `saved` is the last downloaded script, compared by IDENTITY, never deep.
...
export function coalesceKey(action: ScriptAction): string | null {
  switch (action.type) {
    case "EDIT_TEXT":
...
function asSavedIfUnchanged(present: Script, saved: Script): Script {
  if (present === saved) return present;
  return SCRIPT_FIELDS.every(
    (field) => present[field as keyof Script] === saved[field as keyof Script]
  )
    ? saved
    : present;
}

export function initHistory(script: Script): HistoryState {
  return { present: script, past: [], future: [], lastKey: null, saved: script };
}

export function historyReducer(state: HistoryState, action: ScriptAction): HistoryState {
  if (action.type === "UNDO") {
