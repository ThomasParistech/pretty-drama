...
// Every import here is pure, so this module runs under `node --test`; locale.ts would
// not be (it reads `window` on import).

import { firstFreeColor, isPaletteColor } from "../shared/characterColors.ts";
import { DEFAULT_LOCALE, isLocale } from "../shared/i18n.ts";
import { isPlayId } from "../shared/plays.ts";
import type { Act, Character, Line, Scene, Script } from "../shared/types.ts";

// Every action the Editor can dispatch, as a DISCRIMINATED UNION: the payload of each
// one is checked at the dispatch site, which is the whole reason the reducer stays the
// single door to a new script. Undo/redo and the save marker are history.ts's own
// actions and travel through the same dispatch, hence the last four.
export type ScriptAction =
  | { type: "LOAD_SCRIPT"; script: unknown }
  | { type: "SET_TITLE"; title: string }
  | { type: "SET_LANGUAGE"; language: string }
  | { type: "ADD_CHARACTER"; id: string; name: string }
  | { type: "SET_CHARACTER_COLOR"; id: string; color: string }
  | { type: "RENAME_CHARACTER"; id: string; name: string }
  | { type: "DELETE_CHARACTER"; id: string; mode: "reassign" | "deleteLines"; reassignTo?: string | null }
  | { type: "ADD_ACT" }
  | { type: "DELETE_ACT"; actIndex: number }
  | { type: "ADD_SCENE"; actIndex: number }
  | { type: "DELETE_SCENE"; actIndex: number; sceneIndex: number }
  | { type: "MOVE_ACT"; from: number; to: number }
  | { type: "MOVE_SCENE"; actIndex: number; from: number; to: number }
  | { type: "ADD_LINE"; actIndex: number; sceneIndex: number; afterLineId: string | null; id: string }
  | { type: "EDIT_TEXT"; actIndex: number; sceneIndex: number; lineId: string; text: string }
  | { type: "SET_LINE_TEXTS"; edits: { lineId: string; text: string }[] }
  | {
  ...
  | { type: "DELETE_LINE"; actIndex: number; sceneIndex: number; lineId: string }
  | { type: "MOVE_LINE"; actIndex: number; sceneIndex: number; activeId: string; overId: string }
  | { type: "UNDO" }
  | { type: "REDO" }
  | { type: "MARK_SAVED" }
  | { type: "HISTORY_BREAK" };
...
export function sanitizeScript(input: unknown): Script {
  // Cast ONCE, here: everything below tests each field before using it, which is the
  // whole job of this function, and `any` at the door beats a cast per field.
  const raw: any =
  ...
  const sanitizeLine = (l: any): Line | null => {
    ...
    if (characterId && characterRemap.has(characterId)) {
      characterId = characterRemap.get(characterId)!;
    }
                  ...
                  ? act.scenes
                      ...
                      .map((scene: any): Scene => ({
                        lines: Array.isArray(scene.lines)
                          ? scene.lines.map(sanitizeLine).filter((l: Line | null) => l !== null)
                          : [],
                      }))
                  : [{ lines: [] }],
            }))
...
export function allLines(script: Script): Line[] {
...
function defaultCharacterId(scene: Scene, idx: number, characters: Character[]): string | null {
...
// STATE as is, because history.ts recognises a refused action by identity. Otherwise
...
function mapAllLines(state: Script, fn: (line: Line) => Line | null): Script {
  return {
    ...state,
    acts: state.acts.map((act) => ({
      ...
      scenes: act.scenes.map((scene) => ({
        ...scene,
        lines: scene.lines.map(fn).filter((l): l is Line => l !== null),
      })),
    })),
  };
}
...
function applyTextEdits(state: Script, edits: { lineId: string; text: string }[]): Script {
...
function moved<T>(list: T[], from: number, to: number): T[] | null {
  const ok = (i: number) => Number.isInteger(i) && i >= 0 && i < list.length;
...
export function scriptReducer(state: Script, action: ScriptAction): Script {
  switch (action.type) {
    case "LOAD_SCRIPT":
    ...
    case "DELETE_CHARACTER": {
      ...
      let next = state;
      // Read out of the action before the closure: a narrowing does not survive into
      // a callback.
      const reassignTo = action.reassignTo;
      if (action.mode === "reassign" && reassignTo != null) {
        next = mapAllLines(next, (l) =>
          l.characterId === action.id ? { ...l, characterId: reassignTo } : l
        );
      } else {
