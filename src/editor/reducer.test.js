...
test("Editing and Speaking share fill in colours identically", () => {
  // THE contract with characterColors.js: a script with no colours is filled in by
...
test("a rejected action returns the EXACT state it was given, not a copy", () => {
  // history.js pushes by identity: a copy births empty steps and lights the label.
  const before = play();
...
test("the play's identifier enters the compared fields by itself", () => {
  // history.js derives SCRIPT_FIELDS from `Object.keys(EMPTY_SCRIPT)`, so a new field
