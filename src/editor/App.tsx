...
import { fetchScript, downloadBlob, githubUploadUrl, HttpError } from "../shared/data.ts";
import { isPlayId } from "../shared/plays.ts";
import { EMPTY_SCRIPT, allLines, newId, indexAfterMove, indexAfterRemoval } from "./reducer.ts";
import { historyReducer, initHistory } from "./history.ts";
import PlayHeader from "../shared/PlayHeader.tsx";
import { fmt, t, translator } from "../shared/locale.ts";
import T from "../shared/T.tsx";
import { actLabel } from "../shared/structureLabels.ts";
import { pageLabelKey } from "../shared/pages.ts";
import { UndoIcon, RedoIcon, WarnIcon } from "../shared/icons.tsx";
...
import useTouchPointer from "./useTouchPointer.ts";
import "./editor.css";
import type { FocusRequest } from "./LineRow.tsx";
import type { RailSection } from "./EditorRail.tsx";
import type { Match } from "./search.ts";
import type { Character } from "../shared/types.ts";

// The character whose deletion is being settled, and what it costs in lines.
...
export default function App() {
  ...
  const addLine = useCallback(
    (actIndex: number, sceneIndex: number, afterLineId: string | null) => {
      const id = newId();
  ...
  const goToScene = (ai: number, si: number) => {
    setActIndex(ai);
    setSceneIndex(si);
  };
  ...
  const deleteScene = (ai: number, si: number) => {
  ...
  const moveAct = (from: number, to: number) => {
    dispatch({ type: "MOVE_ACT", from, to });
    setActIndex(indexAfterMove(safeActIndex, from, to));
  };
  const moveScene = (ai: number, from: number, to: number) => {
  ...
  const [railSection, setRailSection] = useState<RailSection | null>("structure");
  ...
  const goToMatch = useCallback((match: Match, focus?: boolean) => {
      ...
      {uploadNotice && (
        <ConfirmModal
          ...
          onPrimary={() => {
            ...
            download();
            window.open(uploadUrl ?? undefined, "_blank", "noopener,noreferrer");
          }}
...
function DeleteCharacterModal({
  ...
  onCancel: () => void;
  onConfirm: (mode: "reassign" | "deleteLines", reassignTo?: string | null) => void;
}) {
