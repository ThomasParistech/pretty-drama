...
import useScrollToActiveCard from "../shared/useScrollToActiveCard.ts";
import PlayHeader from "../shared/PlayHeader.tsx";
import ProgressBar from "../shared/ProgressBar.tsx";
import LeaveGuard from "../shared/LeaveGuard.tsx";
...
import useRecorder, { extensionForMimeType } from "./useRecorder.ts";
import type { Character, ManifestLine } from "../shared/types.ts";
import "./recorder.css";
...
const MIC_ERROR_KEY: Record<string, string> = { mic: "recorder.micError" };

// A take of THIS session: in memory only, one per line, never persisted.
interface Take {
  blob: Blob;
  ext: string;
  text: string;
...
function countLines(
  lines: ManifestLine[],
  characterId: string,
  isTodo: (line: ManifestLine) => boolean
): Counts {
...
function optionSuffix(counts: Counts | null): string {
...
export default function App() {
  ...
  const isTodo = useCallback((line: ManifestLine) => lineState(line) === "todo", [lineState]);
  ...
  const saveTake = (line: ManifestLine, blob: Blob | null, mimeType: string) => {
  ...
  const deleteTake = (line: ManifestLine) => {
    setTakes((prev) => {
  ...
  const downloadZip = async () => {
    // The button is rendered past the loading guard below, so this only re-states for
    // the compiler what the render order already guarantees.
    if (!manifest) return;
    const zip = new JSZip();
  ...
  const selectLine = (line: ManifestLine) => {
...
function waveHeights(seed: string, count = WAVE_BARS): number[] {
  let h = 0;
...
async function decodePeaks(src: string, count = WAVE_BARS): Promise<number[]> {
...
function pauseOtherAudio(except?: EventTarget | null) {
    ...
    >
      ...
      <audio
        ...
        onLoadedMetadata={(e) => {
          ...
          if (!Number.isFinite(e.currentTarget.duration)) e.currentTarget.currentTime = 1e7;
        }}
        onDurationChange={(e) => {
          const d = e.currentTarget.duration;
          if (Number.isFinite(d)) {
            setDuration(d);
            if (e.currentTarget.currentTime > d) e.currentTarget.currentTime = 0;
          }
        }}
      />
    </div>
