...
import PlayHeader from "../shared/PlayHeader.tsx";
import PageState from "../shared/PageState.tsx";
import UploadTile from "../shared/UploadTile.tsx";
import useManifest from "../shared/useManifest.ts";
import { actLabel, sceneLabel } from "../shared/structureLabels.ts";
import { fmt, formatWhen, t } from "../shared/locale.ts";
import { pageLabelKey } from "../shared/pages.ts";
import T from "../shared/T.tsx";
import { CheckIcon, CrossIcon, DownloadIcon, WarnIcon } from "../shared/icons.tsx";
import { githubUploadUrl, slugify } from "../shared/data.ts";
...
// One journal row, normalised by `filesOf`: whatever the manifest held, these are the
// fields the table draws.
interface JournalRow {
  file: string;
  kind: "voix" | "script" | "inconnu";
  error: string | null;
  clips: number;
  changes: ScriptChanges | null;
}

// A cell of the grid, and the shape both the scene and the character totals take.
interface Tally {
  ok: number;
  total: number;
}
...
function filesOf(run: HistoryRun): JournalRow[] {
  // `any` per file: a journal written by an older Action, or hand-edited, can hold
  // anything, and every test below is the normalisation this function exists for.
  const files: any[] = run && Array.isArray(run.files) ? run.files : [];
  return files
...
const CHANGE_LABEL_KEYS: Record<string, string> = {
...
function changesOf(changes: ScriptChanges): string {
  const parts: string[] = [];
  for (const [field, key] of Object.entries(CHANGE_LABEL_KEYS)) {
    const value = changes[field as keyof ScriptChanges];
    if (value === true) parts.push(t(key));
    // `typeof` only to narrow, `Number.isFinite` to JUDGE, and both are needed: this
    // journal is committed, so hand-editable, and `JSON.parse` turns `1e999` into
    // Infinity, which passes `> 0` and renders as "∞" in the row. `NaN` fails `> 0`.
    else if (typeof value === "number" && Number.isFinite(value) && value > 0) {
      parts.push(t(key, { count: value }));
    }
  }
...
function detailOf(row: JournalRow) {
  if (row.error) {
    return (
      <T
...
function okCount(lines: ManifestLine[]): number {
...
function statusClass(ok: number, total: number): string {
...
function HeadMark({ ok, total }: Tally) {
...
function CellMark({ ok, total }: Tally) {
...
function Journal({ runs }: { runs: HistoryRun[] }) {
...
function KindMark({ kind }: { kind: JournalRow["kind"] }) {
...
function UploadLinks({ playId }: { playId: string }) {
...
function ScriptPdfLink({ title }: { title: string }) {
...
function ProgressTable({
  ...
  acts: { key: number; title: string; span: number; total: number; ok: number }[];
  scenes: {
    key: string;
    act: string;
    label: string;
  ...
  rows: { character: Character; cells: Tally[]; total: number; ok: number }[];
}) {
  ...
  const nameCorner = useRef<HTMLTableCellElement>(null);
  ...
  return (
    ...
    >
      {/* A CSS custom property is not in `CSSProperties`, hence the cast: the frozen
          status column reads it as its `left`. */}
      <table
        className="dash-table"
        style={{ "--dash-name-w": `${nameWidth}px` } as CSSProperties}
      >
        <thead>
          <tr>
