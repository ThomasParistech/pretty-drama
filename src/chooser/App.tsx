...
import { HttpError, fetchPlaysIndex, fetchUnroutedHistory } from "../shared/data.ts";
import { WarnIcon } from "../shared/icons.tsx";
import { fmt, formatWhen, t } from "../shared/locale.ts";
import { playHref } from "../shared/pages.ts";
import { isPlayId } from "../shared/plays.ts";
import { formatShare } from "../shared/share.ts";
import NewPlay, { NewPlayTile } from "./NewPlay.tsx";
import "../home/home.css";
import "./chooser.css";
import type { HistoryRun, JournalFile, PlayEntry } from "../shared/types.ts";

// Both root pages. No link leads from the chooser to management, which is bookmarked.
export default function App({ manage = false }: { manage?: boolean }) {
  const [plays, setPlays] = useState<PlayEntry[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [unrouted, setUnrouted] = useState<HistoryRun[]>([]);
...
function PlayStats({ play, manage }: { play: PlayEntry; manage: boolean }) {
...
function PlayCard({ play, manage }: { play: PlayEntry; manage: boolean }) {
  return (
    <a
...
function Unrouted({ runs }: { runs: HistoryRun[] }) {
  // `filesOf` (dashboard/App.tsx) is deliberately not reused: it also normalises type and
  // clip count for a four-column row this record does not draw.
  // `any` per file: the root journal is committed, hence hand-editable, and the two
  // tests below are the whole normalisation this record needs.
  const files = runs.flatMap((run) =>
    ((Array.isArray(run?.files) ? run.files : []) as any[])
      .filter((file) => file && typeof file.file === "string")
      .map((file): JournalFile & { at: string } => ({ ...file, at: run.at }))
  );
