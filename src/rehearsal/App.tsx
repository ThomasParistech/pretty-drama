...
import { myLineNumbers, myLineNumber, actChoices, sceneChoices } from "../shared/data.ts";
import {
  PlayIcon,
  PauseIcon,
  PrevIcon,
...
export default function App() {
  ...
  const engineRef = useRef<{
    ...
    muet: boolean;
    bip: boolean;
  }>({ lines: [], characterId: "", muet: false, bip: true });
  engineRef.current = { lines, characterId, muet, bip };

  const isMine = useCallback(
    (line: ManifestLine) => characterId !== "" && line.characterId === characterId,
    [characterId]
  );
  ...
  const playAt = (i: number) => {
    ...
    if (line.status === "ok" && line.clip) {
      // The element is rendered unconditionally below, so it exists whenever a line
      // can be played.
