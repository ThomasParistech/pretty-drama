...
import { assignColors } from "../shared/characterColors.ts";
...
import { formatShare } from "../shared/share.ts";
...
import type { BlockRect, Scope, SpeechRow } from "./stats.ts";

// What every legend and drawing needs to word and colour a bucket, passed down from the
// page so the three drawings answer ONE choice.
interface LegendProps {
  rows: SpeechRow[];
  colorOf: (id: string) => string | null;
  // Takes a PARTIAL row: the mosaic's label falls back to an empty one.
  nameOf: (row: Partial<SpeechRow>) => string;
  highlight: string | null;
  pinned: string | null;
}

// The two donuts read one quantity off a row; the mosaic reads none.
type RowValue = (row: SpeechRow) => number;
...
// All the maths is in stats.ts, pure and tested; this file only draws, and is checked by eye.
...
function Stats({ manifest }: { manifest: Manifest }) {
  ...
  const colorOf = (id: string) => (id === UNKNOWN ? null : colors.get(id) ?? null);
  // Two fallbacks: a cast member may also have no name, which is not an orphan line.
  const nameOf = (row: Partial<SpeechRow>) =>
    row.name?.trim()
      ? row.name
  ...
  const firstScene = (index: number) => ((acts[index]?.scenes?.length ?? 0) > 0 ? 0 : ALL);
  const changeAct = (value: number) => {
    setActIndex(value);
...
function CharacterLegend({
  ...
  // No `value` means no numbers, which is the bar under the header.
  value?: RowValue;
  total?: number;
  flow?: boolean;
}) {
            ...
            >
              ...
              {value && (
                <>
                  ...
                  <span className="stats-legend-share">
                    {formatShare(value(row), total ?? 0, t, fmt)}
                  </span>
                </>
              )}
            </button>
...
function Donut({
  ...
  let offset = 0;
  const slices: { row: SpeechRow; fraction: number; offset: number }[] = [];
...
function Timeline({
  ...
  block: { rects: BlockRect[]; columns: number; rows: number };
  where: string;
}) {
