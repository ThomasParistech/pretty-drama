...
export function romanNumeral(n: unknown): string {
  // `typeof` first only to narrow for the compiler: `Number.isInteger` already
  // answers no to everything that is not a number.
  if (typeof n !== "number" || !Number.isInteger(n) || n < 1 || n > 39) return String(n);
  return TENS[Math.floor(n / 10)] + UNITS[n % 10];
}

// `index` is 0-based; the label is 1-based.
export function actLabel(t: Translate, index: number): string {
  return t("structure.act", { n: romanNumeral(index + 1) });
}

export function sceneLabel(t: Translate, index: number): string {
