...
// Covered by `node --test`, so it never imports locale.ts: `t` arrives as an argument.

import type { History, Manifest, PlayEntry, Script, Translate } from "./types.ts";
...
export async function fetchJson<T>(relativeUrl: string): Promise<T> {
...
export function fetchScript() {
  return fetchJson<Script>("data/script.json");
}

export function fetchPlaysIndex() {
  return fetchJson<{ plays?: PlayEntry[] }>("data/plays.json");
}
...
export async function fetchUnroutedHistory(): Promise<History> {
  try {
    return await fetchJson<History>("data/history.json");
  } catch (err) {
...
export function myLineNumbers(
  lines: readonly { id: string; characterId: string | null }[],
  characterId: string
): Map<string, number> {
...
export function myLineNumber(t: Translate, numbers: Map<string, number>, lineId: string): string {
...
export function sceneChoices(
  scenes: readonly { lines: readonly { characterId: string | null }[] }[],
  characterId: string
): number[] {
...
export function actChoices(
  acts: readonly { scenes: readonly { lines: readonly { characterId: string | null }[] }[] }[],
  characterId: string
): number[] {
  return sceneChoices(
...
export function setBeforeUnloadGuard(enabled: boolean): void {
...
// play's journal.
export function githubUploadUrl(playId?: string): string | null {
...
export function githubNewPlayUrl(playId: string, title: string, note: string): string | null {
  const repo = githubRepoUrl();
  if (!repo) return null;
