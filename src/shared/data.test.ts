...
const atUrl = (href: string) => {
  const { hostname, pathname } = new URL(href);
  // The two fields `githubRepoUrl` reads, and nothing else: a whole Location would be
  // scaffolding for a module that only ever touches these.
  globalThis.window = { location: { hostname, pathname } } as unknown as Window &
    typeof globalThis;
};
