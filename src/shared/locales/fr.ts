...
// { one, other } selected from `params.count` (see i18n.ts). Placeholders are
// `{name}`.
//
...
// user-visible literal survives in a .tsx outside this folder (accented text,
// text-bearing attributes, JSX text nodes). locales/parity.test.ts holds it to
// en.ts.
//
...
export const FR: Catalogue = {
  ...
  // pages.ts, and changing it would make an already validated text drift. The
  ...
  // structureLabels.ts). `{n}` is a roman numeral for an act and a digit for a
  ...
  // was never spoken. `characterColors.ts` carries the keys, not the words, so that
  ...
  // The chosen scope, composed by `scopeText` (stats/App.tsx).
  ...
  // (see the comment above `<Journal>` in dashboard/App.tsx), it needs to be reachable
  ...
  // distinction as everywhere else on the site (see `HttpError` in data.ts).
  "chooser.loadError":
