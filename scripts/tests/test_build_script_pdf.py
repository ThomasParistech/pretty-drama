"""The play's PDF script, checked on the .tex `render_tex` produces: it cannot fail
the deployment, and no text typed into the editor can break the compilation.
Tests that really compile skip without an engine."""
...
# No act or scene title on purpose: a written one would make the "Acte I"
# assertions pass by coincidence.
SCRIPT = {
...
class TestLatexEscape(unittest.TestCase):
    ...
    def test_backslash_is_not_escaped_twice(self):
        # Escaping "\" first would have later passes escape its replacement again.
    ...
    def test_accents_and_french_quotes_pass_through(self):
        # UTF-8 .tex read by inputenc: nothing to escape, nothing to transliterate.
    ...
    def test_a_blank_line_never_becomes_a_paragraph_break(self):
        # A blank line is a \par; \lhead and \MakeUppercase stop dead on one and the
        # whole PDF disappears silently, hence the flattening.
...
class TestRenderTex(unittest.TestCase):
    ...
    def test_act_and_scene_headings(self):
        # Composed by the module, not read from script.json.
        tex = render_tex(SCRIPT)
    ...
    def test_a_leftover_title_in_the_file_is_ignored(self):
        # Older files carry titles; two ways of naming a scene is one too many.
        script = {
            "title": "T",
            "language": "fr",
    ...
    def test_an_english_play_is_composed_in_english(self):
        # The play's language, not the reader's. TestStructureLabels holds the words
        # themselves to the front-end catalogues.
    ...
    def test_babel_follows_the_play_language(self):
        # `english.ldf` ships with texlive-latex-base; a third needs its own package.
    ...
    def test_an_unknown_language_falls_back_to_french(self):
        # Mirror of `sanitize_script` and of the front end.
    ...
    def test_rule_separates_scenes_but_never_opens_an_act(self):
        # Act I has two scenes (one rule), act II one (none). Body only: the preamble
        # holds the \newcommand.
    ...
    def test_new_act_opens_a_page_but_not_the_first(self):
        # \clearpage, not \newpage: in two columns the latter only moves right.
        tex = render_tex(SCRIPT)
...
class TestRenderTexTolerance(unittest.TestCase):
    """script.json is hand-editable: degrade like build_manifest, never raise."""
    ...
    def test_the_empty_notice_follows_the_play_language(self):
        # Everything this module writes of its own accord follows the play.
    ...
    def test_missing_title_gets_a_placeholder(self):
        # An empty \lhead{\textit{}} would go unnoticed until printing.
...
class TestCompile(unittest.TestCase):
    def test_compile_failure_never_raises(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # The failure tails the LaTeX log to stderr; wanted in CI, not here.
    ...
    def test_a_hand_edited_blank_line_still_compiles(self):
        # Without the flattening, LaTeX stops on the \par and the PDF vanishes.
        script = {
            "title": "Transport\n\nde Femmes",
